...
function SortableChannelItem({ channel, isActive, onRemove, isOverlay }: { channel: Channel, isActive: boolean, onRemove: (e: React.MouseEvent) => void, isOverlay?: boolean }) {
    ...
    return (
        <li ref={setNodeRef} style={style} className={`text-sm group rounded-lg overflow-hidden transition-all ${isOverlay ? 'shadow-2xl scale-105 z-50 bg-neutral-900 border border-indigo-500/50' : ''}`}>
            <Link
                href={`/?channelId=${channel.id}`}
                className={`flex items-center justify-between px-3 py-2 w-full ${isActive
                    ? 'bg-indigo-50 text-indigo-700 border border-indigo-200 dark:bg-indigo-900/40 dark:text-indigo-200 dark:border-indigo-500/20 shadow-sm'
    ...
    return (
        <div className="flex flex-col gap-1">
            ...
            {category.id !== 'uncategorized' && (
                <div className="group flex items-center justify-between px-2 py-1 text-neutral-400 hover:text-indigo-300 transition-colors">
                    {editingCategoryId === category.id ? (
                        <div className="flex items-center gap-2 flex-1">
                            <input
                                autoFocus
                                type="text"
                                className="bg-neutral-800 text-white text-xs px-1 rounded w-full border-indigo-500 border"
                        ...
                        <div className="flex items-center gap-2 text-xs font-semibold tracking-wide uppercase flex-1">
                            <span onDoubleClick={() => startEditingCategory(category.id, category.name)}>{category.name}</span>
            ...
            >
                <ul
                    ref={setNodeRef}
                    className={`space-y-0.5 min-h-[10px] rounded-lg p-1 transition-colors border ${isOver
        ...
        <div className="flex flex-col gap-6 h-full">
            ...
            <div className="flex flex-col gap-2">
                <Link
                    ...
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group text-sm ${isHome
                        ? 'bg-indigo-600/10 text-indigo-700 dark:bg-indigo-600/20 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30'
                        : 'text-neutral-600 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-white/5 hover:text-neutral-900 dark:hover:text-white'
                        }`}
                >
                    <Home size={18} className={isHome ? "text-indigo-600 dark:text-indigo-400" : "group-hover:text-neutral-900 dark:group-hover:text-white transition-colors"} />
            ...
            >
                <div className="flex flex-col gap-4 overflow-y-auto pb-10 scrollbar-thin scrollbar-thumb-neutral-800 scrollbar-track-transparent pr-2 relative flex-1">
                    <div className="flex items-center justify-between px-2">
                        <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Channels</h3>
                        <button
                        ...
                        <form onSubmit={handleCreateCategory} className="px-1 animate-in slide-in-from-top-2">
                            <div className="flex gap-2">
                                <input
                                    autoFocus
                                    type="text"
                                    className="flex-1 bg-neutral-900 border border-neutral-700 rounded p-1 text-sm text-white"
                                    placeholder="Category Name"
                                    value={newCategoryName}
                                    onChange={e => setNewCategoryName(e.target.value)}
                                    onBlur={() => !newCategoryName && setIsAddingCategory(false)}
                                />
                                <button type="submit" className="text-indigo-400"><Check size={16} /></button>
                            </div>
                        </form>
                    )}
            ...
            <form onSubmit={handleSearchChannels} className="relative group px-1 mb-2 mt-auto">
                <input
                    ...
                    className="w-full bg-white dark:bg-black/40 border border-neutral-200 dark:border-neutral-800 rounded-lg pl-3 pr-8 py-2 text-sm text-neutral-900 dark:text-neutral-300 placeholder-neutral-400 dark:placeholder-neutral-600 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all"
                />
                <button
                    type="submit"
                    disabled={!channelQuery.trim() || isSearching}
                    className={`absolute right-2 top-1/2 -translate-y-1/2 p-1 transition-colors ${isSearching ? 'animate-pulse text-indigo-500' : 'text-neutral-500 hover:text-indigo-400'}`}
                >
                    <Search size={16} />
                </button>
            </form>
            {channelResults.length > 0 && (
                <div className="absolute bottom-16 left-4 right-4 z-50 bg-neutral-900 border border-neutral-700 rounded-xl p-2 shadow-2xl max-h-[300px] overflow-y-auto">
